...
annolite(
  data = list(
    paragraphs = kauder_speech_flist,
...
emma_chapters <- lapply(
  ...
  function(ch){
    fulltextlist_paras <- lapply(ch, fulltextlist, element = "p")
    do.call(c, fulltextlist_paras)
  }
)
```
...
emma_chapters <- lapply(
  ...
  function(i){
    name(emma_chapters[[i]]) <- sprintf("Chapter %s", i)
    emma_chapters[[i]]
  }
)
```
...
```{r}
emma_chapters[[2]][[1]][["attributes"]][["name"]]
```
